...
plot(prod_c, pch = 18, col = "black", xlab = "Index", ylab = "Sales", cex = 1.5, ylim = c(800, 1800))
...
```{r roll_corr_plot, message=FALSE, warning=FALSE, out.width = '100%'}
...
alt_data_roll_corr %>%
  ...
  facet_wrap(~ Coin, ncol = 3, scales = "free_y") +
  # Aesthetics
  scale_color_tq() +
  labs(
...
The above figure shows some significagnt patters in the rolling correlations of the daily return for all alt coins in consideration. What is also interesting is the fact that these patters are very similar for all alt coins under study. 

Next we are going to decompose these time series into their basic components 1) Seasonality 2) Trends and 3) Random components. Decomposing a time series is a common practice that helps reveal seasonal effects and trends. A very good introduction about time series decomposition for the non-technical can be found [here](https://anomaly.io/seasonal-trend-decomposition-in-r/).

We will start by trying to detect the seasonality in the time series as follows
