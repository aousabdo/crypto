---
title: A Quick Look at Crypto Currencies with R - Part 1 Downloading and Processing
  Crypto Data with R
...
```{r}
kable(head(btc_usd), "latex", booktabs=T) %>% column_spec(1, bold = T) %>% kable_as_image()
```
...
```{r}
kable(tail(btc_usd), "latex", booktabs=T) %>% column_spec(1, bold = T) %>% kable_as_image()
```
...
```{r}
kable(head(tmp2), "latex", booktabs=T) %>% column_spec(1, bold = T) %>% kable_as_image()
```
